...
- [ ] **Step 1: 후보 생성 + 지표 + 실측 (`experiments/prefilter_bench.py candidates`)**
...
- [ ] **Step 2: 판별력 분석 (`prefilter_bench.py analyze`)**
...
- [ ] **Step 3: 종단 비교 (`prefilter_bench.py e2e`)**
  - 전략 4종: (a) degeneracy ≤ 8, (b) treewidth ≤ 454, (c) treewidth ≤ T*(Step 2 보정), (d) degeneracy ≤ D*(보정, 공정 비교용).
  ...
  - 그래프: Step 1 과 같은 16 그래프 × h2/h2+3 × 축약 on/off, `RunSpec.run_seed`, num_reads=100.
  - 기록: n_subgraphs, target_k, qubo_vars_max, elapsed, apsp_sum, strongly_connected, 그리고
    최종 분할 서브그래프 전부의 minorminer 실측(60 초) → `partition_embeddable` 비율.
...
## Risks & Rollback
- **Risks:**
  - minorminer 시간 제한 실패가 "탐색 실패"일 수 있음(#93 주의). 경계 후보는 실패 시 1 회 재시도(seed 1).
