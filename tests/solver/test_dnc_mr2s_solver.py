...
from tests.util.graph_fixtures import directed_edges_dict, directed_tuples
...
class StubEmbeddingAwareRunningMr2sSolver(StubRunningMr2sSolver):
  ...
  def run_with_embedding(
    ...
    return Solution(
      edges={
        edge.id: Edge(edge.vertices[1], edge.vertices[0], edge.weight, True)
        for edge in graph.edges.values()
      },
      graph=graph,
      sample_set=empty_binary_sample_set(),
...
def test_merge_solutions_combines_solution_edges() -> None:
  ...
  merged = solver.merge_solutions(
    solutions=[
      Solution(
        edges=directed_edges_dict({(1, 2), (2, 3)}),
        graph=Graph(edges=[Edge(1, 2, 1, False), Edge(2, 3, 1, False)]),
        sample_set=sample_set,
      ),
      Solution(
        edges=directed_edges_dict({(2, 3), (4, 3)}),
  ...
  assert directed_tuples(merged.edges) == {(1, 2), (2, 3), (4, 3)}
...
def test_merge_solutions_keeps_one_direction_per_input_edge() -> None:
  ...
  merged = solver.merge_solutions(
    solutions=[
      ...
      Solution(
        edges=directed_edges_dict({(2, 1), (3, 2)}),
        graph=graph,
        sample_set=empty_binary_sample_set(),
      ),
    ],
  ...
  selected_undirected_edges = {
    ...
    for source, target in directed_tuples(merged.edges)
  }

  assert len(merged.edges) == len(graph.edges)
  assert selected_undirected_edges == {
...
def test_merge_solutions_selects_direction_that_reduces_flow_imbalance() -> None:
  ...
  merged = solver.merge_solutions(
    solutions=[
      Solution(
        edges=directed_edges_dict({(1, 3), (2, 3)}),
        graph=graph,
        sample_set=empty_binary_sample_set(),
      ),
      Solution(
        edges=directed_edges_dict({(3, 2)}),
        graph=graph,
        sample_set=empty_binary_sample_set(),
      ),
    ],
  ...
  assert (1, 3) in directed_tuples(merged.edges)
  assert (3, 2) in directed_tuples(merged.edges)
...
def test_solve_subgraphs_skips_qubo_solver_for_directed_only_graph() -> None:
  ...
  assert directed_tuples(solutions[0].edges) == {(1, 2), (2, 3)}
...
def test_solve_subgraphs_reuses_matching_physical_embedding() -> None:
  ...
  assert [directed_tuples(solution.edges) for solution in solutions] == [{(2, 1)}, {(4, 3)}]
...
def test_solve_subgraphs_falls_back_for_placeholder_embedding() -> None:
  ...
  assert directed_tuples(solutions[0].edges) == {(1, 2)}
...
def test_run_solves_full_graph_after_applying_merged_directions(
  ...
  child_edge = graph.edge_for_endpoints(1, 2)
  assert child_edge.directed is True
  assert child_edge.vertices == (1, 2)
  remaining_edge_in_graph = graph.edge_for_endpoints(*remaining.endpoints())
  assert remaining_edge_in_graph.directed is False
  assert directed_tuples(solution.edges) == {(1, 2), (2, 3)}
