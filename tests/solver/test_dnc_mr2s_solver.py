...
def _edge_id_for_endpoints(graph: Graph, u: int, v: int) -> int:
    ...
    ids = [edge.id for edge in graph.edges.values() if edge.endpoints() == endpoints]
    assert len(ids) == 1
    return ids[0]
...
class StubEmbeddingAwareRunningMr2sSolver(StubRunningMr2sSolver):
    ...
    def run_with_embedding(
        ...
        return Solution(
            edges={
                edge.id: (edge.vertices[1], edge.vertices[0])
...
class RealEvaluatorMr2sSolver:
    ...
    def build_bqm(self, graph: Graph) -> QuboMatrix:
        raise NotImplementedError(
            "RealEvaluatorMr2sSolver.build_bqm must not be called"
        )
...
def test_score_merged_solution_measures_strong_connectivity_on_merged_orientation() -> (
    None
):
...
def test_solve_subgraphs_reuses_matching_physical_embedding() -> None:
    ...
    assert [set(solution.edges.values()) for solution in solutions] == [
        {(2, 1)},
        {(4, 3)},
    ]
...
def test_degeneracy_pruning_partition_strategy_does_not_call_embedding_estimator() -> (
    None
):
    graph = Graph(
        edges=[
...
def test_replacing_default_partition_strategy_after_init_does_not_sync_or_raise() -> (
    None
):
    graph = Graph(edges=[Edge(1, 2, 1, False)])
...
def test_divide_graph_uses_mutated_face_cycle() -> None:
    ...
    solver.face_cycle = cast(
        FaceClusterPartition, StubFaceCycle(sub_graphs=[new_child])
    )
