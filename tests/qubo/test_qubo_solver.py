...
def test_sample_logs_debug_context_for_empty_sample_set(caplog) -> None:
    ...
    qubo = BinaryQuadraticModel(
        {"e_1_2": 1.0}, {("e_1_2", "e_2_3"): -1.0}, 0.0, "BINARY"
    )
...
def test_run_returns_directed_solution_without_sampling_when_no_undirected_edges() -> (
    None
):
    ...
    assert set(solution.edges.values()) == {(2, 1), (2, 3)}
    assert solution.graph is graph
    assert len(solution.sample_set) == 0


def test_run_returns_default_solution_without_sampling_when_qubo_has_no_variables() -> (
    None
):
    solver = QuboSolver(ranker=ApspSumRanker(), sampler=FailingSampler())
...
def test_select_best_sample_returns_default_solution_for_empty_sample_set() -> None:
    ...
    assert set(solution_edges.values()) == {(1, 2), (2, 3)}


def test_run_with_embedding_returns_directed_solution_without_sampling_when_no_undirected_edges() -> (
    None
):
    solver = QuboSolver(ranker=ApspSumRanker(), sampler=FailingSampler())
    qubo = BinaryQuadraticModel({}, {}, 0.0, "BINARY")
...
def test_run_with_embedding_returns_default_solution_when_qubo_has_no_variables() -> (
    None
):
    solver = QuboSolver(ranker=ApspSumRanker(), sampler=FailingSampler())
...
def test_run_with_embedding_returns_default_solution_when_sampler_returns_empty_sample_set(
    monkeypatch,
) -> None:
...
def test_run_with_embedding_stores_fixed_embedding_composite(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        qubo_solver_module,
        "select_best_sample",
        lambda sample_set, edges, ranker: {0: (1, 2)},
    )
    solver = QuboSolver(
        ranker=ApspSumRanker(),
        sampler=FailingSampler(),
