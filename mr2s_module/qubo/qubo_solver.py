...
class QuboSolver:
    ...
    def create_sa_solver(
        ...
        return QuboSolver(
            ranker=ranker, sampler=SimulatedAnnealingSampler(), num_reads=num_reads
        )
    ...
    def run(self, qubo: QuboMatrix, graph: Graph) -> Solution:
        ...
        return Solution(
            edges=select_best_sample(
                sample_set, list(graph.edges.values()), self.ranker
            ),
    ...
    def _sample(
        ...
        if len(sample_set) == 0:
            logger.warning(
                "SampleSet is empty; sample_set.info=%s; %s",
                sample_set.info,
    ...
    def _sample_debug_info(
        ...
        qubo_variable_count = (
            len(qubo_variables) if qubo_variables is not None else "unknown"
        )
