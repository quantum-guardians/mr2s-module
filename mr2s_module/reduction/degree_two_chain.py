...
class ChainReport:
    ...
    equal_endpoint_weight_chains: (
        int  # 첫·끝 간선 가중치가 같은 path 수 (축약 정확도 자료)
    )
...
def _walk_chain(
    ...
    fwd_edges, fwd_passed, fwd_terminal = _traverse(
        start, first_edge, incident, interior
    )
    if fwd_terminal == start:
        # 전체 컴포넌트가 사이클: 한 바퀴 돌아 시작점 복귀. start 가 끝점 역할.
        return _make_chain((start, start), tuple(fwd_passed), fwd_edges)

    bwd_edges, bwd_passed, bwd_terminal = _traverse(
        start, second_edge, incident, interior
    )
...
def _build_report(graph: Graph, chains: tuple[Chain, ...]) -> ChainReport:
    equal_weight = sum(
        ...
        and graph.edges[chain.edge_ids[0]].weight
        == graph.edges[chain.edge_ids[-1]].weight
    )
    return ChainReport(
        chains=chains,
