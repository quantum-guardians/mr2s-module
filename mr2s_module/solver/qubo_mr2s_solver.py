...
class QuboMR2SSolver:
    def __init__(
        self,
        qubo_solver: QuboSolverProtocol = QuboSolver.create_sa_solver(
            ranker=ApspSumRanker()
        ),
        evaluator: EvaluatorProtocol = Evaluator(),
        poly_generators: list[PolyGeneratorProtocol]
        | set[PolyGeneratorProtocol]
        | None = None,
    ) -> None:
        if poly_generators is None:
            poly_generators = [
                FlowPolyGenerator(),
                NHopPolyGenerator(
                    small_world_spec=SmallWorldSpec(n_hops=[NHop(2, 1), NHop(3, 1)])
                ),
            ]
    ...
    def _build_polynomial(self, graph: Graph) -> BinaryPolynomial:
